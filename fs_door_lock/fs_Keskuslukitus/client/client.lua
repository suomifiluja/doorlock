                    ...
                    if(string.lower(plate) == localVehPlate)then
                        ...
                        if(vehicle ~= "locked")then
                            ...
                            if(time > timer)then
                                ...
                                TriggerEvent("ls:notify", "Sinun pitää odottaa " .. (timer / 1000) .." sekunttia")
                            end
                        else
                            TriggerEvent("ls:notify", "Avaimet eivät ole autossa!")
                        end
                    end
                end
                ...
                if(not hasKey)then
                    ...
                    if(isInside)then
                        ...
                        if(canSteal())then
                            ...
                            TriggerEvent("ls:notify", "Avaimet eivät ole ajoneuvossa!")
                        end
                    end
                end
end
...
AddEventHandler("ls:getHasOwner", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    if(not hasOwner)then
        ...
        TriggerEvent("ls:notify", "Tämä ajoneuvo ei ole sinun!")
    end
end)
...
function Notify(text, duration)
	if(globalConf['CLIENT'].notification)then
		if(globalConf['CLIENT'].notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_GANGAPP", "CHAR_GANGAPP", true, 1, "Keskuslukitus", "           ", duration)
			DrawNotification_4(false, true)
		elseif(globalConf['CLIENT'].notification == 2)then
			TriggerEvent('chatMessage', '^1Keskuslukitus', {255, 255, 255}, text)
		else
			return
		end
	else
